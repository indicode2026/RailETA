...
`;
  ...
  const stations = data.elements.map(item => {
    ...
    return {
      id: item.id,
      name: tags.name || tags["name:en"] || tags.short_name || "Railway Station",
      code: tags["railway:ref"] || tags.ref || tags["uic_ref"] || "",
      lat: itemLat,
      lon: itemLon,
      railwayType: tags.railway,
      distance: haversineKm(lat, lon, itemLat, itemLon),
      // Defensive exclusion for objects incorrectly tagged as railway stations.
      metroLike: /metro|subway/i.test(tags.name || "") ||
                 /^(subway|light_rail)$/i.test(tags.railway || "") ||
                 /subway/i.test(tags.station || "")
    };
  })
  .filter(s => Number.isFinite(s.lat) && Number.isFinite(s.lon) && !s.metroLike)
  ...
  for (const station of stations) {
    const key = `${station.name.toLowerCase()}|${station.lat.toFixed(4)}|${station.lon.toFixed(4)}`;
    if (!seen.has(key)) {
      seen.add(key);
      unique.push(station);
    }
  ...
  stations.forEach((station, index) => {
    ...
    button.innerHTML = `
      ...
      <div class="station-main">
        ...
        <span>${station.code ? escapeHtml(station.code) : "Railway station"} · ${station.distance.toFixed(1)} km away</span>
      </div>
      <div class="arrow">→</div>
    `;
  ...
  navigator.geolocation.getCurrentPosition(async position => {
    ...
    try {
      ...
      els.locationSummary.textContent =
        `Found ${stations.length} nearby railway station${stations.length === 1 ? "" : "s"} around ${userCoords.lat.toFixed(3)}, ${userCoords.lon.toFixed(3)}.`;
...
function selectStation(station) {
  ...
  els.selectedStationCode.textContent = station.code || "Railway station";
